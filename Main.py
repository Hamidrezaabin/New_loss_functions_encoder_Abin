...
#         raise Exception("Google Drive mount failed")
...
def train(model, device, train_loader, optimizer, epoch, LR):
    ...
    for batch_idx, (m, x, z, y, magnitude, syndrome) in enumerate(
        ...
        loss, x_pred = model.loss_new_mmd(-z_pred, z_mul.to(device), y.to(device), kernel="polynomial",sigma=1.0, degree=3, coef0=1.0)
        model.zero_grad()
        loss.backward()
        optimizer.step()
        ###
    ...
    with torch.no_grad():
        for ii, test_loader in enumerate(test_loader_list):
            ...
            while True:
                ...
                z_pred = model(magnitude.to(device), syndrome.to(device))
                # x_2=bin_to_sign(x)
                # x_pred_2=-z_pred * torch.sign(y)
                
                #loss, x_pred = model.loss(-z_pred, z_mul.to(device), y.to(device))
                loss, x_pred = model.loss_new_mmd(-z_pred, z_mul.to(device), y.to(device), kernel="polynomial",sigma=1.0, degree=3, coef0=1.0)
                #rr1 = torch.cat([x, x_2, y, x_pred_2, x_pred], dim=0)  # shape: (10,)
                # if ii==0:
                #     matrix_list_0.append(rr1)
                # elif ii==1:
                #     matrix_list_1.append(rr1)
                # else:
                #     matrix_list_2.append(rr1)
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if loss < best_loss: 
            best_loss = loss 
            #torch.save(model, os.path.join(args.path2, 'best_model_LDPC_newsize.pt'))
            torch.save(model, os.path.join(args.path, 'best_model_LDPC_newsize.pt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=2)#1000
    ...
    print(type(code.generator_matrix))
    torch.save(code.generator_matrix, f'G_{code.code_type}{code.n},{code.k}.pt')
    code.pc_matrix = torch.from_numpy(H).long()
    torch.save(code.pc_matrix, f'H_{code.code_type}{code.n},{code.n-code.k}.pt')
    args.code = code
    ####################################################################
    model_dir = os.path.join('Results_ECCT_LDPC',
                             args.code_type + '__Code_n_' + str(
                                 args.code_n) + '_k_' + str(
                                 args.code_k) + '__' + datetime.now().strftime(
                                 "%d_%m_%Y_%H_%M_%S"))
    #drive_path ='/media/student/data/Abin/ECCT-LDPC/newsize' 
    # drive_path='D:/python-codes/python-new/codes_results_polar-jalali-pc/ECCT/ECCT'
                          
    #model_dir2 = os.path.join(drive_path,
                              # args.code_type + '__Code_n_' + str(
                                  # args.code_n) + '_k_' + str(
                                  # args.code_k) + '__' + datetime.now().strftime(
                                   #"%d_%m_%Y_%H_%M_%S"))                                
