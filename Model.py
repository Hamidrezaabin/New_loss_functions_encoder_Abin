...
class ECC_Transformer(nn.Module):
    ...
    def gaussian_kernel(self,x, y, sigma=1.0):
        ...
        dist_sq = torch.sum((x - y) ** 2, dim=-1)  # (B,B)
        return torch.exp(-dist_sq / (2 * sigma ** 2)) 


    def polynomial_kernel(self, x, y, degree=2, coef0=1.0):
   
        return (x @ y.T + coef0) ** degree 


    def mmd_loss(self, x, y, kernel="gaussian", sigma=1.0, degree=2, coef0=1.0):
        ...
        if kernel == "gaussian":
            ...
            Kxx = self.polynomial_kernel(x, x, degree, coef0)
            Kyy = self.polynomial_kernel(y, y, degree, coef0)
            Kxy = self.polynomial_kernel(x, y, degree, coef0)
        else:
                raise ValueError("kernel must be 'gaussian' or 'polynomial'")
        ...
        return mmd_sq  # or just return mmd_sq
    
    def loss_new_mmd(self, z_pred, z2, y, kernel="gaussian",sigma=1.0, degree=3, coef0=1.0):
        tt2=sign_to_bin(torch.sign(z2))
        loss = self.mmd_loss(z_pred,tt2,kernel=kernel,sigma=sigma, degree=degree, coef0=coef0 )
