...
from Distance_implementations import compute_llr
...
def main(args):
    ...
    model=torch.load('D:/python-codes/python-new/jalali-LDPC/ken_asking/codes_for_test_train/Test_ECCT/best_model_polar_n_64_k_32_epochs_1000.pt',map_location=torch.device('cpu'), weights_only=False)
    model.to(device)
...
def decoder_classic_polar(H,sigma,y,max_iter2,x):
    ...
    x_hat= belief_propagation(H, llr, max_iter=max_iter2)
    ...
    for ii in range(3):
        ...
        for j in range(args.test_records):
            ...
            # print(f'nural_mmd_g={nural_mmd_g}')
             ...
             classic_decode,Hamming_classic=decoder_classic_polar(H,sigma2[ii],bb2,max_iter,bb3)
